...
void gefp_engine::cache_binomial_coeff(void)
{
...
void gefp_engine:: reformulate_to_bagofdistances(void)
{
...
double gefp_engine::norm_gfp(std::vector <int> & query_v)
{
...
void gefp_engine::matching_gfp(std::vector <int> &query_v)
{
	int middleidx = max_bow_len/2;
...
void gefp_engine::matching_bow(std::vector <int> &query_v )
{
...
void gefp_engine::query(int dtype, std::vector <int> &query_v, std::vector < std::pair <double, int> > **scoreoutput)
{
	//~ avoids any skip 
	start_l = 0; 
	stop_l = 0;
...
void gefp_engine::run_evaluation(int dtype)
{
...
void gefp_engine::build_tfidf(void)
{
	//~ find id size, maxlen
	int maxid= -1;
	int maxid_idx = -1;
...
void gefp_engine::insert_wordscan(std::vector <int> scanbow, std::vector <double> xpos, std::vector <double> ypos)
{
...
int gefp_engine::read_wordscan_file(std::string filename)
{
